...
export default function DashboardEdit(props) {
    useEffect(() => {
        const dashboard = props.dashboards.find((c) => (c.id === props.editingId));
        const state = { ...fields };
        state.name.value = dashboard.name;
        setFields(state);
    }, [props.editingId])
    ...
    const handleSubmit = async (e) => {
        ...
        let res = await fetch(`http://localhost:8080/api/dashboard/${props.editingId}`, { method: "post", body: form })
