...
export default function OrderAdd(props) {
	useEffect(() => {
		const order = props.orders.find((o) => o.id === props.editingId);
		const state = { ...fields };
		state.status.value = order.status;
		setFields(state);
	...
	const handleSubmit = async (e) => {
		...
		if (orderItems.length <= 0) return setFeedback(`É necessário ter pelo menos 1 produto no carrinho.`);
		...
		let res = await fetch(`http://localhost:8080/api/order/${props.editingId}`, { method: "post", body: form })
			.then((res) => res.json())
			.then((res) => res);
					...
					{orderItems.length > 0 ? (
						orderItems.map((p) => {
							return (
								<div className="product_list_row__col">
									<div className="product_list_row__item">
										<span className="product_list_row__name">{p.product.name}</span>
										<span className="product_list_row__price">
